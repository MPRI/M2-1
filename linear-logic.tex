...
\paragraph{Règles structurelles} Il y en a 3 :
...
Commentaires :
\begin{itemize}
\item La \emph{contraction} exprime l'immuabilité de la vérité : elle permet d'utiliser des hypothèses plusieurs fois. 
\item L'\emph{affaiblissement} permet élargir les hypothèses ; (permet de faire des manips sur le système lui-même ??)
\item La règle d'\emph{échange} exprime la commutativité des séquents.
\end{itemize}
Ces règles peuvent paraître anodines, elles semblent tomber sous le sens. Pourtant, elles vont se révéler très importantes. (Sauf la règle d'échange : on considèrera que les séquents sont des multi-ensembles non-ordonnés de formules.)\footnote{Toutefois, il y a une connexion entre réseaux de preuve non commutatifs et syllogismes aristotéliciens : voir par ex. Le point aveugle}
